...
| 2026-07-28 | Dispatcher chosen at dispatch time by the button pressed; card face renders the spec union | The 0.5.0 stored-dispatcher picker confused within a day (behavior hidden in a dropdown). `CardStore.beginDispatch` records choice + queued→dispatched in one transaction so the run record can't disagree with what ran; `DispatcherParamSpec.placement` splits the union into face / Advanced / hidden (resume session = chip, never a field) |
